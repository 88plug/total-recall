...
def test_upsert_progress_bumps_last_progress_ts(conn: sqlite3.Connection):
    ...
    goals_idx.upsert_from_extractions(conn, [_ext("goal_progress", "Done. things.", progress)])
    row = conn.execute("SELECT last_progress_ts FROM goal_stack WHERE goal_text='X'").fetchone()
...
def test_upsert_status_hint_done_marks_done(conn: sqlite3.Connection):
    ...
    row = conn.execute("SELECT status FROM goal_stack WHERE goal_text='X done now'").fetchone()
...
def test_upsert_status_hint_blocked_marks_blocked(conn: sqlite3.Connection):
    ...
    row = conn.execute("SELECT status FROM goal_stack WHERE goal_text='X is stuck'").fetchone()
...
def test_active_to_paused_after_30_days(conn: sqlite3.Connection):
    ...
    assert (
        conn.execute("SELECT status FROM goal_stack WHERE goal_text='G'").fetchone()[0] == "active"
    )
    ...
    assert (
        conn.execute("SELECT status FROM goal_stack WHERE goal_text='G'").fetchone()[0] == "paused"
    )
...
def test_paused_to_abandoned_after_60_days(conn: sqlite3.Connection):
    ...
    assert (
        conn.execute("SELECT status FROM goal_stack WHERE goal_text='G'").fetchone()[0]
        == "abandoned"
    )
...
def test_done_is_terminal(conn: sqlite3.Connection):
    ...
    assert (
        conn.execute("SELECT status FROM goal_stack WHERE goal_text='G done'").fetchone()[0]
        == "done"
    )
...
def test_blocked_to_active_on_fresh_progress(conn: sqlite3.Connection):
    ...
    goals_idx.upsert_from_extractions(conn, [_ext("goal_progress", "Done.", t0 + 5 * 86_400)])
...
def test_progress_resets_pause_clock(conn: sqlite3.Connection):
    ...
    t0 = 1_700_000_000
    goals_idx.upsert_from_extractions(conn, [_ext("goal", "G", t0)])
    goals_idx.upsert_from_extractions(conn, [_ext("goal_progress", "Shipped.", t0 + 45 * 86_400)])
