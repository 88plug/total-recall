...
def _project_record_for_dir(proj_dir: Path) -> ProjectRecord | None:
    ...
    sessions_by_mtime = sorted(sessions, key=lambda p: p.stat().st_mtime if p.exists() else 0)
...
def _extract_machines_from_text(text: str, ts: int, machines: dict[str, MachineRecord]) -> None:
...
def extract_ontology(
    ...
    now = max((p.last_active_ts for p in snap.projects), default=0) or 0
...
def extract_ontology_from_records(records: Iterable[Any]) -> OntologySnapshot:
    ...
    for rec in records:
        ...
        if raw_dict is not None:
            ...
            session_id = raw_dict.get("sessionId") or raw_dict.get("session_id") or "_stream"
        else:
            # Record object path.
            ...
            if ts_attr is not None:
                ...
                ts_val = ""
            session_id = getattr(rec_obj, "session_id", None) or "_stream"
