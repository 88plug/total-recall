...
- **gemma4 profile** (matched before generic gemma): JSON refine uses
  `temp=0.1 top_p=0.95 top_k=64 think=false` (card free-form is 1.0/0.95/64;
  Ollama structured-output practice is cooler for schema).
- **CPU bakeoff knobs:** `TOTAL_RECALL_OLLAMA_NUM_GPU=0` + optional
  `TOTAL_RECALL_OLLAMA_NUM_THREAD`.
- **Bakeoff harness** `scripts/eval_llm_bakeoff.py` + `docs/eval-llm-bakeoff.md`.
- Result (card settings): **`qwen3.5:2b` still wins** GPU and CPU on composite
  quality (def_cov 1.0, echo 0.0) at ~20–50× lower latency than gemma4 12b.
  Embed stays `qwen3-embedding:0.6b`. MTP not present on stock Linux GGUF tags.
