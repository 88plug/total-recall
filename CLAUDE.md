...
**Current state: v0.3 operator-aware surfaces shipping.** The repo is no longer greenfield — the pipeline (walker → extractors → SQLite/FTS5/vec index → hooks/MCP/skills/commands delivery) is wired end-to-end and shipping as a Claude Code plugin. Trajectory:
...
- `extractors/` — `decisions`, `corrections`, `self_corrections`, `progress`, `domain_facts`, `away_summaries`, `secrets` (scrubber).
- `index/` — SQLite/FTS5 store, `ingest.py` (incremental, inode-aware), `query.py`.
- `vec/` — optional `sqlite-vec` + `fastembed` layer, RRF fusion with FTS5.
- `mcp_server/` — FastMCP stdio server: core tools (`tools.py`) + resources.
- `mcp_server/extras/` — v0.3 operator-aware tool surfaces, one file per domain (`bans_tools.py`, `corrections_tools.py`, `decisions_tools.py`, `escalation_tools.py`, `goals_tools.py`, `ontology_tools.py`, `operator_context_tools.py`, `operator_tools.py`, `rhetoric_tools.py`, `voice_tools.py`).
...
- **Docker validation harness** — `Dockerfile.test` builds a Python 3.11-slim image with `jq`, `bash`, `sqlite3`, `mcp`, `click`, `fastembed`, `sqlite-vec` pre-installed; agents mount the source via `-v` and run the full test matrix inside the container. A 10-agent validation pass over this harness surfaced 16 issues (HIGH/MEDIUM/LOW); regression tests for those fixes live in `tests/integration/test_corpus.py` and `tests/integration/test_golden_path.py`. The integration test file `tests/integration/test_golden_path.py` is the canonical entry point for end-to-end validation — every user-visible surface (cold-start ingest, SessionStart signpost, UserPromptSubmit retrieval, MCP recall) has at least one test there.

**v0.3 surfaces to exercise during validation:** the 23 MCP tools (6 core in `mcp_server/tools.py` + 17 operator-aware in `mcp_server/extras/`); the `total-recall metrics {summary,cost,sessions,topics,health}` CLI; and `get_operator_context` as the single SessionStart bundler the v2 signpost calls instead of issuing many small queries.
